...
void IKMonitor::on_ShowAllPOTNodesCheckBox_stateChanged(int state)
{
    ...
    ui()->ShowPOTNode8CheckBox->setChecked(checked);
    ui()->ShowPOTNode9CheckBox->setChecked(checked);
...
//                        if(POTSystem[k]->isCalibrated())
    ...
    TestMode = 2;
                            ...
                            _3DViewer->updateBonesOriWithIKresult(_3DViewer->IKResult);//, Vector3D(-1,0,0));
...
//Set IK/FK Tpose Calibrate Flag
        ...
        Q4IK.Qinit_LeftElbow  = Q4FK.Qinit_LeftElbow  = pot[5]->getOri();
        Q4IK.Qinit_RightElbow = Q4FK.Qinit_RightElbow = pot[6]->getOri();
        Q4IK.Qinit_LeftThigh  = Q4FK.Qinit_LeftThigh = pot[7]->getOri();
        Q4IK.Qinit_RightThigh = Q4FK.Qinit_RightThigh = pot[8]->getOri();
        ...
        P4IK.Pinit_LeftElbow  = P4FK.Pinit_LeftElbow  = pot[5]->getPos();
        P4IK.Pinit_RightElbow = P4FK.Pinit_RightElbow = pot[6]->getPos();
        P4IK.Pinit_LeftThigh  = P4FK.Pinit_LeftThigh = pot[7]->getPos();
        P4IK.Pinit_RightThigh = P4FK.Pinit_RightThigh = pot[8]->getPos();
...
void IKMonitor::on_ConnectDefaultPOTComPortsButton_clicked()
{
    ...
    foreach (const QSerialPortInfo &info, QSerialPortInfo::availablePorts())
    {
        ...
        if ( info.description().contains("CH340",Qt::CaseInsensitive) ||
             info.description().contains("G-Wearables",Qt::CaseInsensitive)  )
        {
            if ( n < 7 )
            {
