...
void InputAdaptor::setResultBufferQ(int id, Quaternion Q)
{
    ...
    case 7:
        ...
        return;
    case 8:
        resultBuffer.Q_LeftThigh = Q;
        return;
    case 9:
        resultBuffer.Q_RightThigh = Q;
        return;
    default:
        break;
    }
...
///LEGS/////////////////////////////////////////////////////////////////////
    case 3:
        Din.id = HumanFK::Joint_LeftAnkle;

        Din.Qthis   = resultBuffer.Q_LeftAnkle * humanFK.getQinit(HumanFK::Joint_LeftAnkle).conj();    // debugLeftArm.QThis  =
        Din.Qlinker = resultBuffer.Q_LeftThigh * humanFK.getQinit(HumanFK::Joint_LeftThigh).conj();    // debugLeftArm.QLinker=
        Din.Qroot   = resultBuffer.Q_Hip;  //* humanFK.getQinit(HumanFK::Joint_Neck).conj();           // debugLeftArm.QRoot =
        Din.Proot   = resultBuffer.fkResult_Hip;                                                       // debugLeftArm.PNeck =
        humanFK.compute(Din); //humanFK.computeForDebug (Din, debugLeftArm.PNeck, debugLeftArm.PShoulder, debugLeftArm.PElbow, debugLeftArm.PWrist);
        offset = Quaternion::rotVbyQ(humanFK.getOffset(HumanFK::Joint_LeftAnkle), Din.Qthis);
        resultBuffer.fkResult_LeftAnkle = humanFK.output().LeftAnklePos ;
        P = resultBuffer.fkResult_LeftAnkle - offset;
        break;
    case 4:
        Din.id = HumanFK::Joint_RightAnkle;

        Din.Qthis   = resultBuffer.Q_RightAnkle * humanFK.getQinit(HumanFK::Joint_RightAnkle).conj();
        Din.Qlinker = resultBuffer.Q_RightThigh * humanFK.getQinit(HumanFK::Joint_RightThigh).conj();
        Din.Qroot   = resultBuffer.Q_Hip;  //* humanFK.getQinit(HumanFK::Joint_Neck).conj();
        Din.Proot   = resultBuffer.fkResult_Hip;
        humanFK.compute(Din);
        offset = Quaternion::rotVbyQ(humanFK.getOffset(HumanFK::Joint_RightAnkle), Din.Qthis);
        resultBuffer.fkResult_RightAnkle = humanFK.output().RightAnklePos ;
        P = resultBuffer.fkResult_RightAnkle - offset;
        break;
///////////////////////////////////////////////////////////////////////
    ...
    case 5:
        ...
        Din.Qroot = resultBuffer.Q_Hip = ik.output().HipQuat;
        Din.Proot = resultBuffer.fkResult_Hip = ik.output().HipPos;
        humanFK.compute(Din);
...
void InputAdaptor::computeIK(int id, HumanIK& ik, int Mode, int InputSrc)
{
    ...
    if(Mode == 1)
    {
        if(id == 1)
        {
            ...
            if(InputSrc == 0)                   //if(resultBuffer.isMissing_LeftWrist)
                rawData.Pos = resultBuffer.fkP_LeftWrist;  //P
            else if(InputSrc == 1)              //else
            ...
            resultBuffer.fkResult_Hip = ik.output().HipPos;
            resultBuffer.Q_Hip = ik.output().HipQuat;
#endif
        }
        else if(id == 2)
        {
        ...
        else if(id ==3 )
        {
            ...
            if(InputSrc == 0)
                rawData.Pos = resultBuffer.fkP_LeftAnkle;
            else if(InputSrc == 1)
