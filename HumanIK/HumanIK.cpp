...
{//--1:Whole body --2: UpperBody
    ...
    if(Mode == 1)
    {
        ...
        Model.LenHipToThigh = 0.5 * (Pinit.Pinit_LeftThigh.getX() - Pinit.Pinit_RightThigh.getX());

        Float AnkleHeight = 0.07 ;

        dLAnkle = Vector3D( dNeck.getX() + Model.LenHipToThigh, AnkleHeight, dNeck.getZ());
        dRAnkle = Vector3D( dNeck.getX() - Model.LenHipToThigh, AnkleHeight, dNeck.getZ());
...
/*    else if(Mode == 3)
    {
        ...
        dNeck = Vector3D(0.5 * (Pinit.Pinit_LeftThigh.getX() + Pinit.Pinit_RightThigh.getX()), mNeck.getY() + Model.LenHipToChest + Model.LenChestToNeck, 0.5 * (Pinit.Pinit_LeftThigh.getX() + Pinit.Pinit_RightThigh.getX()));
        Model.LenHipToThigh = 0.5 * (Pinit.Pinit_LeftThigh.getX() - Pinit.Pinit_RightThigh.getX());
        Float AnkleHeight = 0.07 ;
        dLAnkle = Vector3D( dNeck.getX() + Model.LenHipToThigh, AnkleHeight, dNeck.getZ());
        dRAnkle = Vector3D( dNeck.getX() - Model.LenHipToThigh, AnkleHeight, dNeck.getZ());

        TOffset.offset_LeftWrist = Vector3D(0,0,0);//dLWrist - mLWrist;
        TOffset.offset_RightWrist = Vector3D(0,0,0);//dRWrist - mRWrist;
        TOffset.offset_LeftAnkle = dLAnkle - mLAnkle;
        TOffset.offset_RightAnkle = dRAnkle - mRAnkle;
        TOffset.offset_Neck = dNeck - mNeck;
...
/////////TestTestTestTestTest//////////END////////////////////
 ...
 //   output.RightThighOri.setRoll(output.HipOri.getRoll());
    ...
    QW = ik->getJoints()[0].getQwBWD();
    QL = HipQWInv*QW ;
    output.LeftThighRootOri =  EulerAngle(0,0,0);    //ouQL.toEuler().RadToDegree() ;
    output.LeftThighRootQuat =  Quaternion(1,0,0,0) ;//QL ;
    ...
    QW = ik->getJoints()[0].getQwBWD();
    QL = HipQWInv*QW ;
    output.RightThighRootOri  = EulerAngle(0,0,0);    //QL.toEuler().RadToDegree() ;
    output.RightThighRootQuat =  Quaternion(1,0,0,0) ;//QL ;
#endif
